<template>
    <div class="bg-white col-span-4 w-full py-16 px-20 pb-20 shrink-1" v-if="showModule">
        ...
        <div class="text-6xl font-medium mb-16" v-if="quizItem">
            ...
            <div class="flex flex-col lg:flex-row gap-12" v-if="quizItem.type == 'img'">
                <div class="relative cursor-pointer w-full" v-for="(content, ind) in quizItem.content" :key="ind" @click="changeActiveElementQuiz(ind)">
                    ...
                    <div class="absolute text-4xl text-[#fff] inset-12 flex items-end" v-html="content.title"></div>
                </div>
            </div>
            <div class="flex flex-col flex-nowrap" v-else-if="quizItem.type == 'text'">
                <div class="relative cursor-pointer w-full flex mb-5 p-2 hover:bg-[#00755012]" v-for="(content, ind) in quizItem.content" :key="ind" @click="changeActiveElementQuiz(ind)">
